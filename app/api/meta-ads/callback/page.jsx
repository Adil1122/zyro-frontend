    ...
    if (code && state) {
      ...
      .then(response => {
        if (response.ok) {
          ...
          router.push('/marketing?meta_ads_connected=true');
        } else {
          // Error - redirect with error message
          router.push('/marketing?error=callback_failed');
        }
      })
      .catch(error => {
        console.error('Meta Ads callback error:', error);
        router.push('/marketing?error=callback_error');
      });
    } else {
      // Missing parameters
