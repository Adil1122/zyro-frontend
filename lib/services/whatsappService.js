...
export const whatsappService = {
     ...
     */
    async sendMetaWhatsAppTemplate({ phoneNumberId, accessToken, recipientPhone, templateName, params, languageCode = 'en' }) {
        ...
        if (!res.ok) {
            ...
            // If template not found in 'en', retry with 'en_US'
            if (languageCode === 'en' && errBody?.error?.code === 132001) {
                console.log(`[WhatsApp Service] Template not found in 'en'. Retrying with 'en_US'...`);
            ...
            // Self-healing: If number of parameters doesn't match, retry with NO parameters (empty array)
            if (params && params.length > 0 && errBody?.error?.code === 132000) {
                console.warn(`[WhatsApp Service] Parameter mismatch (expected 0, got ${params.length}). Retrying with NO parameters...`);
                return this.sendMetaWhatsAppTemplate({
                    phoneNumberId,
                    accessToken,
                    recipientPhone,
    ...
    async sendOrderStatusUpdate(userId, orderNumber, newStatus, customerPhone, customerName, totalAmount) {
        try {
            const { data: user } = await supabase
                .from('users')
                .select('wa_phone_number_id, wa_access_token, wa_is_active, currency')
                .eq('id', userId)
                .single();
            ...
            if (status === 'processing') {
                ...
                // Store pending cancellation so webhook can handle YES/NO reply
                const formattedPhone = this.formatPhoneNumber(customerPhone);
                if (formattedPhone) {
                    await supabase.from('wa_pending_orders').insert({
                        user_id: userId,
                    ...
                    }).catch(e => console.error('[WhatsApp] Failed to insert pending_cancellation:', e.message));
                }
    ...
    async sendMerchantOrderAlert(userId, orderNumber, customerName, total) {
        try {
            const { data: user } = await supabase
                .from('users')
                .select('wa_phone_number_id, wa_access_token, wa_is_active, wa_merchant_phone, currency')
                .eq('id', userId)
                .single();
    ...
    async sendLowStockAlert(userId, productName, currentStock) {
        try {
            const { data: user } = await supabase
                .from('users')
                .select('wa_phone_number_id, wa_access_token, wa_is_active, wa_merchant_phone')
                .eq('id', userId)
                .single();
