    ...
    async getWhatsappData(userId) {
        ...
        if (userId) {
            const { data: userRow } = await supabase
                .from('users')
                .select('wa_merchant_phone, wa_phone_number_id')
                .eq('id', userId)
                .maybeSingle();
            merchantPhone = userRow?.wa_merchant_phone || userRow?.wa_phone_number_id || null;
        }
    ...
    async sendOrderCreated(userId, { customerPhone, customerName, orderNumber, total, deliveryAddress, cityName, orderDetail }) {
        try {
            ...
            const { error: pendingErr } = await supabase.from('wa_pending_orders').insert({
                ...
                order_detail: orderDetail || `Order ${orderNumber}`,
