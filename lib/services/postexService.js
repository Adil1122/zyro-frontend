...
export async function createPostExOrder(apiKey, orderData) {
    ...
    try {
        const payload = {
            ...
            customerPhone: orderData.customerPhone,           // 03xxxxxxxxx format (mandatory)
            deliveryAddress: orderData.deliveryAddress,       // Delivery address (mandatory)
            cityName: orderData.cityName,                     // City name (mandatory)
            invoicePayment: String(orderData.invoicePayment), // COD amount as string (mandatory)
            orderType: orderData.orderType || 'Normal',       // Normal / Reversed / Replacement (mandatory)
            pickupAddressCode: orderData.pickupAddressCode || 'OFC', // Merchant pickup address code (mandatory)
            invoiceDivision: orderData.invoiceDivision || 1, // Number of shipments, usually 1 (mandatory)
            items: orderData.items || 1,                     // Number of pieces (mandatory)
