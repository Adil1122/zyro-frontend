...
async function shopifyGet(endpoint, params = {}, creds) {
    ...
    if (!res.ok) {
        ...
        if (res.status === 403 && body.includes('Non-expiring access tokens')) {
...
// Returns data + cursor from Shopify's Link header for proper pagination
async function shopifyGetWithCursor(endpoint, params = {}, creds) {
    const base = getBaseUrl(creds?.domain);
    if (!base) throw new Error('Shopify is not configured');
    ...
    const res = await fetch(url, { method: 'GET', headers: getHeaders(creds?.accessToken) });
    if (!res.ok) throw new Error(`Shopify API error ${res.status}: ${await res.text()}`);

    const data = await res.json();
    const link = res.headers.get('Link') || '';
    const next = (link.match(/<[^>]*[?&]page_info=([^&>\s]+)[^>]*>;\s*rel="next"/) || [])[1] || null;
    const prev = (link.match(/<[^>]*[?&]page_info=([^&>\s]+)[^>]*>;\s*rel="previous"/) || [])[1] || null;

    return { data, nextPageInfo: next, prevPageInfo: prev };
}
...
export async function getShopifyOrders({ page = 1, perPage = 50, search = '', status = 'all', creds, pageInfo = null } = {}) {
    const limit = Math.min(perPage, 250);
    const statusParam = status === 'all' ? 'any' : status;

    const countData = await shopifyGet('/orders/count.json', { status: statusParam }, creds);
    const totalOrders = countData?.count ?? 0;
    const totalPages = Math.max(1, Math.ceil(totalOrders / perPage));

    let result;
    if (pageInfo) {
        // Shopify cursor pagination — only limit allowed alongside page_info
        result = await shopifyGetWithCursor('/orders.json', { limit, page_info: pageInfo }, creds);
    } else {
        const params = { limit, status: statusParam, order: 'created_at desc' };
        if (search) params.name = search;
        result = await shopifyGetWithCursor('/orders.json', params, creds);
    }

    let orders = (result.data?.orders || []).map(normalizeOrder);
...
function normalizeOrder(sOrder) {
    ...
    const lineItems = sOrder.line_items || [];
