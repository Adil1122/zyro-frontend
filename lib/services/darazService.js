...
export async function getDarazOrders({ page = 1, perPage = 10, search = '', status = 'all', credentials } = {}) {
    ...
    // /orders/get does not include item details — fetch them separately in one call
    let itemsByOrderId = {};
    if (rawOrders.length > 0) {
        try {
            const orderIds = rawOrders.map(o => o.order_id);
            const itemsData = await darazGet('/order/items/get', {
                order_ids: JSON.stringify(orderIds),
            }, credentials);
            console.log('[Daraz] /order/items/get raw response type:', typeof itemsData, Array.isArray(itemsData) ? 'array' : 'object', JSON.stringify(itemsData)?.slice(0, 300));
            if (Array.isArray(itemsData)) {
                // Flat array — each item has an order_id field
                ...
                // Object keyed by order_id: { "123456": [{item}, ...], "789012": [...] }
                for (const [orderId, items] of Object.entries(itemsData)) {
                    if (Array.isArray(items)) {
                        itemsByOrderId[String(orderId)] = items;
                    }
                }
            }
        } catch (e) {
            console.warn('[Daraz] Could not fetch order items:', e.message);
        }
    }
...
export async function getDarazOrderById(orderId, credentials) {
    ...
    try {
        const itemsData = await darazGet('/order/items/get', {
            order_ids: JSON.stringify([order.order_id]),
        }, credentials);
        fetchedItems = Array.isArray(itemsData) ? itemsData : undefined;
    } catch { /* fall back to inline items */ }
    return normalizeOrder(order, fetchedItems);
}
