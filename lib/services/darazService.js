...
// Valid Daraz API status values
const ALL_DARAZ_STATUSES = 'unpaid,pending,ready_to_ship,dispatched,delivered,failed_delivery,canceled,returned';

// Map UI filter values to Daraz API status strings
const STATUS_MAP = {
...
export async function getDarazStats(credentials) {
    ...
    const todayData = await darazGet('/orders/get', {
        ...
        status: 'unpaid,pending,ready_to_ship,dispatched,delivered',
        sort_by: 'created_at',
        sort_direction: 'DESC',
        limit: 100,
        offset: 0,
...
export async function getDarazOrders({ page = 1, perPage = 10, search = '', status = 'all', credentials } = {}) {
    ...
    if (status && status !== 'all') {
        ...
        params.status = ALL_DARAZ_STATUSES;
    }

    const data = await darazGet('/orders/get', params, credentials);
