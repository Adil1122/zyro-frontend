...
function StatusBadge({ status }) {
    const s = LEOPARDS_STATUS[status] || LEOPARDS_STATUS["pending"];
    return (
        <span style={{
...
export default function LeopardsManagePage({ onBack }) {
    ...
    const [creds, setCreds] = useState({ apiKey: "", apiPassword: "" });
    ...
    const fetchConfig = async () => {
        try {
            ...
            if (data) {
                setCreds({
                    apiKey:      data.leopards_api_key      || "",
                    apiPassword: data.leopards_api_password || "",
                });
            }
        } catch (e) { console.error("Failed to fetch Leopards config:", e); }
    };
    ...
    const fetchStats = async () => {
        try {
            ...
            setStats(data);
        } catch (e) { console.error("Failed to fetch Leopards stats:", e); }
    };
    ...
    const fetchOrders = useCallback(async (page = 1) => {
        ...
        try {
            ...
            const userId = getCurrentUserId();
            const res  = await fetch(`/api/leopards/orders?${params}`, { headers: { 'x-user-id': userId } });
            const data = await res.json();

            if (!data.configured) {
                setError("Leopards is not configured. Please set your credentials below.");
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('users')
                .update({ leopards_api_key: creds.apiKey, leopards_api_password: creds.apiPassword })
                .eq('id', userId);
    ...
    const handleTrack = async () => {
        ...
        try {
            const userId = getCurrentUserId();
            const res  = await fetch(`/api/leopards/track?cn=${encodeURIComponent(trackingCN.trim())}`, {
                headers: { 'x-user-id': userId },
            });
            setTrackResult(await res.json());
        } catch (e) { setTrackResult({ error: e.message }); }
            ...
            <div style={{ padding: "24px 32px 0", borderBottom: `1px solid ${T.border}`, background: T.bgCard, flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{
                            width: 32, height: 32, borderRadius: T.r8, background: LEOPARDS_GRAD,
                            display: "flex", alignItems: "center", justifyContent: "center",
                            fontSize: 17, boxShadow: "0 2px 8px rgba(26,71,42,0.5)",
                        }}>🐆</div>
                        <div>
                            <div style={{ fontSize: 18, fontWeight: 800, color: T.text, letterSpacing: "-0.4px" }}>
                                Leopards Courier Management
                            </div>
                    ...
                    <div style={{ display: "flex", gap: 24, paddingBottom: 24 }}>
                        {[
                            ...
                            <React.Fragment key={s.label}>
                                <div>
                                    <div style={{ fontSize: 10, color: T.textFaint, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</div>
                ...
                <div style={{ flex: 1, padding: 40, background: T.bgMain, overflow: "auto" }}>
                    <div style={{ maxWidth: 620, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
                        ...
                        <Card style={{ padding: 32 }}>
                            <h3 style={{ fontSize: 20, fontWeight: 800, color: T.text, marginBottom: 6 }}>Leopards Configuration</h3>
                            <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 28 }}>
                                Enter your Leopards Courier Services API credentials to enable shipment booking and tracking.
                            </p>
                            ...
                            <FIELD
                                ...
                                hint="Provided by Leopards Courier Services on account approval"
                            />
                            <FIELD
                                label="API Password"
                            ...
                            {/* Test mode toggle */}
                            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, padding: "12px 14px", borderRadius: T.r8, background: T.bgElev, border: `1px solid ${T.border}` }}>
                                <input
                                    type="checkbox"
                                    id="lcs-testmode"
                                    checked={testMode}
                                ...
                                />
                                <label htmlFor="lcs-testmode" style={{ fontSize: 13, color: T.textSub, cursor: "pointer", fontWeight: 600 }}>
                                    Enable Test Mode
                                </label>
                                <span style={{ fontSize: 12, color: T.textFaint, marginLeft: 4 }}>
                                    Requests go to Leopards sandbox — no real shipments created
                                </span>
                            </div>
                            ...
                            <div style={{ marginTop: 28, padding: 16, background: "rgba(26,71,42,0.06)", borderRadius: T.r10, border: "1px solid rgba(26,71,42,0.2)" }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: "#2d6a4f", marginBottom: 8 }}>How to get API credentials?</div>
                                <ol style={{ fontSize: 12, color: T.textMuted, paddingLeft: 18, lineHeight: 1.7, margin: 0 }}>
                                    <li>Contact <strong>Leopards Courier Services</strong> directly — no self-serve signup.</li>
                                    <li>Call your LCS account representative and request <strong>API access</strong>.</li>
                                    <li>You will receive an <code style={{ fontSize: 11, background: T.bgHigh, padding: "1px 4px", borderRadius: 3 }}>api_key</code> and <code style={{ fontSize: 11, background: T.bgHigh, padding: "1px 4px", borderRadius: 3 }}>api_password</code> pair.</li>
                                    <li>Use <strong>Test Mode</strong> above to test bookings without creating real shipments.</li>
                                </ol>
                            </div>
                        </Card>
                        ...
                        <Card style={{ padding: 24 }}>
                            ...
                            <div style={{ display: "flex" }}>
                                <input
                                    ...
                                    placeholder="Enter Leopards tracking number (10 digits)…"
                ...
                <div style={{ flex: 1, overflow: "auto", background: T.bgMain }}>
                    {error ? (
                        <div style={{ padding: 80, textAlign: "center" }}>
                            ...
                            <GradientButton variant="primary" onClick={() => setIsConfiguring(true)}>Configure Leopards</GradientButton>
                        </div>
                    ) : (
                                        ...
                                        ? (
                                            <tr>
                                                <td colSpan={7} style={{ padding: "60px 40px", textAlign: "center", color: T.textMuted }}>
                                                    No Leopards shipments found. Book your first shipment to get started.
                                                </td>
                                            </tr>
                                        )
                                        : orders.map(order => (
                                            <React.Fragment key={order.id}>
                                                <tr
                                                    ...
                                                    style={{ cursor: "pointer", background: expandedRow === order.id ? T.bgElev : "transparent", transition: "background 0.1s" }}
                                                            ...
                                                            <Card style={{ padding: 0, background: T.bgCard, border: `1px solid ${T.border}`, overflow: "hidden" }}>
                                                                <div style={{ display: "grid", gridTemplateColumns: "300px 1fr" }}>
                                                                    <div style={{ padding: 20, borderRight: `1px solid ${T.border}` }}>
                                                                        <div style={{ marginBottom: 20 }}>
                                                                            ...
                                                                            <div style={{ fontSize: 13, lineHeight: 1.5, color: T.text }}>
                                                                                {order.address.shipping.address1}<br />{order.city}, Pakistan
                                                                            </div>
                                                                        </div>
                                                                        <div style={{ marginBottom: 20 }}>
                                                                    ...
                                                                    <div style={{ display: "flex", flexDirection: "column" }}>
                                                                        <div style={{ padding: "12px 20px", borderBottom: `1px solid ${T.border}`, fontSize: 10, fontWeight: 700, color: T.textFaint, textTransform: "uppercase", background: "rgba(0,0,0,0.02)" }}>
