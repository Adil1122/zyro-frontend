...
function StatusBadge({ status }) {
    const s = MP_STATUS[status] || MP_STATUS["pending"];
    return (
        <span style={{
...
function SkeletonRow() {
    return (
        <tr>{[120, 150, 100, 120, 90, 80, 100].map((w, i) => (
...
export default function MPManagePage({ onBack }) {
    ...
    const fetchConfig = async () => {
        try {
            ...
            const { data } = await supabase.from('users').select('mp_username, mp_password').eq('id', userId).single();
            if (data?.mp_username) setUsernameInput(data.mp_username);
            if (data?.mp_password) setPasswordInput(data.mp_password);
        } catch (e) { console.error("Failed to fetch M&P config:", e); }
    };
    ...
    const fetchStats = async () => {
        try {
            ...
            setStats(data);
        } catch (e) { console.error("Failed to fetch M&P stats:", e); }
    };
    ...
    const fetchOrders = useCallback(async (page = 1) => {
        ...
        try {
            const params = new URLSearchParams({
                page: page.toString(), perPage: pagination.perPage.toString(),
                status: statusFilter, ...(search ? { search } : {}),
            });
            const userId = getCurrentUserId();
            const res  = await fetch(`/api/mp/orders?${params}`, { headers: { 'x-user-id': userId } });
            const data = await res.json();
            if (!data.configured) { setError("M&P is not configured. Please set your credentials below."); return; }
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            const userId = getCurrentUserId();
            const { error } = await supabase.from('users').update({ mp_username: usernameInput, mp_password: passwordInput }).eq('id', userId);
            ...
            <div style={{ padding: "24px 32px 0", borderBottom: `1px solid ${T.border}`, background: T.bgCard, flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: T.r8, background: MP_GRAD, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17 }}>🚐</div>
                        <div>
                            <div style={{ fontSize: 18, fontWeight: 800, color: T.text, letterSpacing: "-0.4px" }}>M&P Courier Management</div>
                ...
                {stats?.configured && (
                    <div style={{ display: "flex", gap: 24, paddingBottom: 24 }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: 18, fontWeight: 800, color: T.yellow, marginTop: 4 }}>{formatCurrency(stats.codPending || 0)}</div>
                        </div>
                        <div style={{ width: 1, background: T.border, margin: "4px 0" }} />
                        <div>
                            <div style={{ fontSize: 10, color: T.textFaint, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Total Recovered</div>
                            <div style={{ fontSize: 18, fontWeight: 800, color: T.green, marginTop: 4 }}>{formatCurrency(stats.codRecovered || 0)}</div>
                        </div>
                    </div>
                )}
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", paddingBottom: 16, flexWrap: "wrap" }}>
                    <form onSubmit={handleSearch} style={{ display: "flex", gap: 0 }}>
                        <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search tracking # or customer…" style={{ padding: "7px 12px", fontSize: 13, background: T.bgElev, border: `1px solid ${T.borderMid}`, borderRight: "none", borderRadius: `${T.r8} 0 0 ${T.r8}`, color: T.text, outline: "none", width: 230, fontFamily: "inherit" }} />
            ...
            {isConfiguring ? (
                <div style={{ flex: 1, padding: 40, background: T.bgMain, overflow: "auto" }}>
                    <Card style={{ maxWidth: 600, margin: "0 auto", padding: 32 }}>
                        <h3 style={{ fontSize: 20, fontWeight: 800, color: T.text, marginBottom: 8 }}>M&P Configuration</h3>
                        <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 24 }}>Enter your M&P merchant credentials provided after account approval.</p>
                        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                <label style={{ fontSize: 13, fontWeight: 700, color: T.textSub }}>Username</label>
                                <input type="text" value={usernameInput} onChange={e => setUsernameInput(e.target.value)} placeholder="Your M&P username" style={{ padding: "10px 14px", borderRadius: T.r8, background: T.bgElev, border: `1px solid ${T.borderMid}`, color: T.text, fontSize: 14, fontFamily: "inherit", outline: "none" }} />
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                <label style={{ fontSize: 13, fontWeight: 700, color: T.textSub }}>Password</label>
                                <input type="password" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} placeholder="Your M&P password" style={{ padding: "10px 14px", borderRadius: T.r8, background: T.bgElev, border: `1px solid ${T.borderMid}`, color: T.text, fontSize: 14, fontFamily: "inherit", outline: "none" }} />
                            </div>
                        </div>
                        <div style={{ display: "flex", gap: 12 }}>
                            <GradientButton variant="primary" onClick={handleSaveConfig} disabled={isSaving || !usernameInput || !passwordInput}>
                        ...
                        <div style={{ marginTop: 32, padding: 16, background: "rgba(92,168,124,0.05)", borderRadius: T.r10, border: `1px solid ${T.border}` }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: T.j200, marginBottom: 6 }}>Where to find credentials?</div>
                            <ol style={{ fontSize: 12, color: T.textMuted, paddingLeft: 18, lineHeight: 1.6 }}>
                                <li>Contact M&P corporate sales at <strong>111-645-111</strong> or <strong>corporate@mp.com.pk</strong>.</li>
                                <li>Request an <strong>e-commerce merchant account</strong> with API access.</li>
                                <li>After approval, M&P will send your <strong>username</strong> and <strong>password</strong>.</li>
                                <li>Minimum volume: ~100–200 shipments/month required.</li>
                            </ol>
                        </div>
                    </Card>
                </div>
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            ...
                            <tbody>
                                {ordersLoading
                                    ...
                                    : orders.length === 0
                                        ? <tr><td colSpan={7} style={{ padding: "60px 40px", textAlign: "center", color: T.textMuted }}>No shipments found.</td></tr>
                                        : orders.map(order => (
                                                            ...
                                                            <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 0 }}>
                                                                <div style={{ padding: 20, borderRight: `1px solid ${T.border}` }}>
                                                                    <div style={{ marginBottom: 20 }}>
                                                                        ...
                                                                        <div style={{ fontSize: 13, lineHeight: 1.5, color: T.text }}>{order.address.shipping.address1}<br />{order.address.shipping.city}, Pakistan</div>
                                                                    </div>
                                                                    <div style={{ marginBottom: 20 }}>
                                                                ...
                                                                <div style={{ display: "flex", flexDirection: "column" }}>
                                                                    <div style={{ padding: "12px 20px", borderBottom: `1px solid ${T.border}`, fontSize: 10, fontWeight: 700, color: T.textFaint, textTransform: "uppercase" }}>Order Items ({order.itemCount})</div>
                                                                    <div style={{ flex: 1 }}>
                                                                        {order.items?.map((item, i) => (
                                                                            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "12px 20px", borderBottom: i < order.items.length - 1 ? `1px solid ${T.border}` : "none" }}>
                                                                                <div>
